...
export default class Messages extends Component {
  ...
  getUserMessages() {
    axios
      .get(`${AppConfig.appUrl}/data/userMessages.js`)
      .then(response => {
  ...
  getSearchResults(e) {
    ...
    const searchMessages = allMessages.filter(
      message =>
      ...
      <div className="messages-wrapper">
        <div className="row mb-30">
          <div className="col-sm-5 col-md-4 col-lg-3">
            <Button
              onClick={() => this.openWriteNewMessageModal()}
              variant="raised"
              color="primary"
              className="text-white btn-lg btn-block mb-10"
            >
              Write new message
            </Button>
          </div>
          ...
          {messages &&
            messages.map((data, key) => (
              <li className="clearfix d-flex" key={key}>
                ...
                />
                <IconButton
                  className={classnames("pull-left mr-15", {
                    "text-warning": data.starred
                  })}
                  onClick={() => this.markMessageAsStar(key)}
                >
                  <i className="ti-star" />
                </IconButton>
                <div className="media pull-left">
                  ...
                  <div className="media-body">
                    ...
                    <div className="mb-10">
                      <Button
                        className="btn-default mr-10 btn-xs"
                        onClick={() => this.onReplyMessage(key)}
                      >
                        <i className="ti-back-right mr-10" /> Reply
                      </Button>
                      <Button
                        className="btn-default btn-xs"
                        onClick={() => this.viewMessage(data)}
                      >
                        <i className="ti-eye mr-10" /> Read
                      </Button>
                    </div>
                    {data.replyBox && (
        ...
        {reloading && <RctSectionLoader />}
        <Modal
          isOpen={this.state.newMessageModal}
          toggle={() => this.toggleWriteNewMessageModal()}
          className={this.props.className}
        >
          <ModalHeader toggle={() => this.toggleWriteNewMessageModal()}>
            Write New Message
          </ModalHeader>
          <ModalBody>
            <FormGroup>
          ...
          <ModalFooter>
            ...
            <Button
              ...
              onClick={() => this.toggleWriteNewMessageModal()}
            >
              Cancel
            </Button>
          </ModalFooter>
        ...
        <Dialog
          open={this.state.viewMessageDialog}
